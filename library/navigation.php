 ...
 */
if ( ! function_exists( 'elitedigitalbase_top_bar_r' ) ) {
	function elitedigitalbase_top_bar_r() {
		wp_nav_menu( array(
			'container'      => false,
 ...
 */
if ( ! function_exists( 'elitedigitalbase_mobile_nav' ) ) {
	function elitedigitalbase_mobile_nav() {
		wp_nav_menu( array(
...
*/
if ( ! function_exists( 'elitedigitalbase_add_menuclass' ) ) {
	function elitedigitalbase_add_menuclass( $ulclass ) {
		$find = array('/<a rel="button"/', '/<a title=".*?" rel="button"/');
		$replace = array('<a rel="button" class="button"', '<a rel="button" class="button"');

		return preg_replace( $find, $replace, $ulclass, 1 );
	}
	add_filter( 'wp_nav_menu','elitedigitalbase_add_menuclass' );
}
...
if ( ! function_exists( 'elitedigitalbase_breadcrumb' ) ) {
	function elitedigitalbase_breadcrumb( $showhome = true, $separatorclass = false ) {
		...
		if ( ! is_front_page() ) {
			...
			if ( is_single() && ! is_attachment() ) {
				...
				// Paginated archives
				echo '<li class="current item-current-' . get_query_var('paged') . '">Page ' . get_query_var('paged') . '</li>';
