...
function elitedigitalbase_pagination() {
	global $wp_query;
...
if ( ! function_exists( 'elitedigitalbase_menu_fallback' ) ) :
function elitedigitalbase_menu_fallback() {
	echo '<div class="alert-box secondary">';
	/* translators: %1$s: link to menus, %2$s: link to customize. */
	printf( __( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'elitedigitalbase' ),
		/* translators: %s: menu url */
		sprintf(  __( '<a href="%s">Menus</a>', 'elitedigitalbase' ),
			get_admin_url( get_current_blog_id(), 'nav-menus.php' )
		),
		/* translators: %s: customize url */
		sprintf(  __( '<a href="%s">Customize</a>', 'elitedigitalbase' ),
...
if ( ! function_exists( 'elitedigitalbase_active_nav_class' ) ) :
function elitedigitalbase_active_nav_class( $classes, $item ) {
	if ( 1 === $item->current || true === $item->current_item_ancestor ) {
		$classes[] = 'active';
	}
	return $classes;
}
add_filter( 'nav_menu_css_class', 'elitedigitalbase_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'elitedigitalbase_active_list_pages_class' ) ) :
function elitedigitalbase_active_list_pages_class( $input ) {
...
add_filter( 'wp_list_pages', 'elitedigitalbase_active_list_pages_class', 10, 2 );
endif;
...
if ( ! function_exists( 'elitedigitalbase_responsive_video_oembed_html' ) ) :
function elitedigitalbase_responsive_video_oembed_html( $html, $url, $attr, $post_id ) {
...
add_filter( 'embed_oembed_html', 'elitedigitalbase_responsive_video_oembed_html', 10, 4 );
endif;
...
if ( ! function_exists( 'elitedigitalbase_mobile_menu_id' ) ) :
function elitedigitalbase_mobile_menu_id() {
...
if ( ! function_exists( 'elitedigitalbase_title_bar_responsive_toggle' ) ) :
function elitedigitalbase_title_bar_responsive_toggle() {
